...
category_colors = {category: mcolors.rgb2hex(cmap(i / len(categories))) for i, category in enumerate(categories)}

df_groups = []
for df_group,date_shift in zip([df_chase[df_chase["Amount"]>0],df_chase[df_chase["Amount"]<=0]],[-5,5]):
    df_group["Transaction Date"] = df_group["Transaction Date"].dt.to_period("M").dt.to_timestamp()
    ...
    df_grouped["Transaction Date"] = df_grouped["Transaction Date"] + pd.DateOffset(days=date_shift)

    df_groups.append(df_grouped)

# calculate the net amount
df_total = df_chase.copy()
df_total["Transaction Date"] = df_total["Transaction Date"].dt.to_period("M").dt.to_timestamp()
...
def update_graph(field, show_total):
    ...
    lim_category = np.max([df_groups[0].groupby("Transaction Date").sum()["Absolute Spend"].max(),
